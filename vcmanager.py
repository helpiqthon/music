from Arab import iqmusic
from Arab.core.managers import edit_delete, edit_or_reply
from Arab.helpers.utils import mentionuser
...
async def get_group_call(chat):
    if isinstance(chat, Channel):
        result = await iqmusic(functions.channels.GetFullChannelRequest(channel=chat))
    elif isinstance(chat, Chat):
        result = await iqmusic(functions.messages.GetFullChatRequest(chat_id=chat.id))
...
async def parse_entity(entity):
    if entity.isnumeric():
        entity = int(entity)
    return await iqmusic.get_entity(entity)


@iqmusic.ar_cmd(pattern="تشغيل_المكالمة")
async def start_vc(event):
    vc_chat = await iqmusic.get_entity(event.chat_id)
...
@iqmusic.ar_cmd(pattern="انهاء_المكالمة")
async def end_vc(event):
    ...
    if not gc_call:
        return
    try:
        await iqmusic(functions.phone.DiscardGroupCallRequest(call=gc_call))
...
@iqmusic.ar_cmd(pattern="معلومات_المكالمة")
...
@iqmusic.ar_cmd(pattern="تسمية_المكالمة?(.*)?")
async def title_vc(event):
    ...
    await iqmusic(functions.phone.EditGroupCallTitleRequest(call=gc_call, title=title))
